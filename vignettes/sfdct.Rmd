...
## without dropping the data frame content ...
...
## Geometry collection is not fully supported

POLYGON triangles in GEOMETRYCOLLECTION will be re-triangulated. 

Points and lines need to be added to the graph. 
...
library(maps)
library(dplyr)
g <- map(plot = FALSE, fill = TRUE) %>% st_as_sf() %>% dplyr::filter(startsWith(ID, "Indonesia")) %>% ct_triangulate() %>% st_geometry() 
#sf::plot_sf(g, col = "aliceblue", main = "")
```
...
data("antarctica")
plot(antarctica)
a <- ct_triangulate(st_difference(antarctica[1], antarctica[2, ]))
...
```{r}
m <- map(plot = FALSE, fill = TRUE) %>% st_as_sf() %>% dplyr::filter(startsWith(ID, "Papua New Guinea"))
plotme <- function(x) {plot(st_geometry(x), col = "aliceblue"); x}
#m %>% ct_triangulate(a = 0.1) %>% plotme() %>% ct_triangulate(a = 1) %>% plotme()
```
