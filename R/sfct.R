...
#' @param convex drop triangles that fall "outside" i.e. "holes" and non-convex regions, \code{FALSE} by default
...
#' @export
#' @name ct_triangulate
ct_triangulate.sfg <- function(x, convex = FALSE, ...){
  if (inherits(x, "POINT") | inherits(x, "MULTIPOINT")) {
    message("no edges, returning with st_triangulate")
    return(st_triangulate(x))
  }
  if (inherits(x, "GEOMETRYCOLLECTION")) {
    warning("GEOMETRYCOLLECTION triangulation not supported, returning empty geometry")
...
#' @export
#' @name ct_triangulate
ct_triangulate.sf <- function(x, convex = FALSE, ...) {
  gtlist <- vector("list", nrow(x))
  gcolname <- attr(x, "sf_column")
  geoms <- st_geometry(x)
  #st_geometry(x) <- NULL
  for (i in seq_along(gtlist)) {
    gtriangles <- st_geometrycollection(ct_triangulate(geoms[[i]], convex = convex,  ...))
    gtlist[[i]] <- gtriangles
  }
  st_geometry(x) <- st_sfc(gtlist)
  x
}
